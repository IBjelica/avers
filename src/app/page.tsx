...
function AversFinancialContent() {
  ...
  const [activeSection, setActiveSection] =
    useState<MenuItem>("menu.ourvalues");
      ...
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          if (menuItem) {
            ...
            // At the top of the page (hero section), hide the underline
            setActiveSection("menu.ourvalues" as MenuItem); // Reset to default
            if (!hoveredItem) {
              updateUnderlinePosition(null);
            }
          }
        }
      });
  ...
  const updateUnderlinePosition = (item: MenuItem | null) => {
    if (!underlineRef.current) return;

    if (!item) {
      // Hide underline when at top of page
    ...
    const containerLeft =
      currentItem.parentElement?.getBoundingClientRect().left || 0;
    ...
    >
      ...
      >
        <div className="container w-[88%] min-w-[320px] mx-auto relative">
          <ul className="flex justify-center gap-x-4 mx-auto text-white text-[clamp(11px,_1.125vw,_24px)] font-light">
            {MENU_ITEMS.map((item) => (
              <li
                key={item}
                className="relative"
        ...
        <div className={`${styles.container} mx-auto`}>
          <div className="flex flex-col md:flex-row items-start justify-evenly">
            ...
            <div className="mt-4 md:w-[40%]">
              <p
                className="text-[clamp(15px,_1.125vw,_20px)] font-light leading-normal text-justify"
                style={{ fontFamily: fontGuide.body.secondary }}
              >
                {t("values.mission")}
              </p>
              <p
                className="text-[clamp(15px,_1.125vw,_20px)] font-light leading-normal mt-[30px] text-justify"
                style={{ fontFamily: fontGuide.body.secondary }}
              >
                {t("values.commitment")}
              </p>
      ...
      <section id="services" className="py-24 bg-gray-100">
        <div className={`${styles.container} mx-auto`}>
          ...
          <div className="space-y-6">
            {Object.entries(
              t("services.categories", { returnObjects: true })
            ).map(
              (
                [key, category]: [string, any] // eslint-disable-line @typescript-eslint/no-explicit-any
              ) => (
                <div
                  key={key}
                ...
                >
                  <button
                    ...
                    className="w-full cursor-pointer p-6 bg-[#53758F] text-white font-semibold text-xl hover:bg-[#53758F]/90 transition-colors flex justify-between items-center"
                  >
                          ...
                          {category.services.map(
                            (service: string, index: number) => (
                              <li key={index} className="flex items-start">
                                <span className="text-[#53758F] mr-3 text-lg">
                                  •
                                </span>
                                <span className="text-[#0E1A28] leading-relaxed">
                                  {service}
                                </span>
                              </li>
                            )
          ...
          <div className="mt-12 text-center">
            <a
              href="#contactus"
              className="bg-[#53758F] hover:bg-[#53758F]/90 text-white px-8 py-3 text-lg"
            >
              {t("menu.contactus")}
            </a>
          </div>
      ...
      <section
        className="py-28 text-white overflow-hidden"
        style={{ backgroundColor: "#53758F" }}
      >
        <div className="w-full mx-auto px-4">
          ...
          <div className="relative -mx-4 sm:-mx-6 md:-mx-8 lg:-mx-16">
            ...
            >
              <CarouselContent
                className="-ml-[clamp(50px,_10vw,_113px)]"
                style={{ fontFamily: fontGuide.body.primary }}
              >
                {[1, 2, 3].map((i) => (
                  <CarouselItem
                    key={i}
                    className="max-w-[658px] max-md:max-w-[90vw] pl-[clamp(20px,_5vw,_113px)] basis-full sm:basis-3/4 md:basis-2/3 lg:basis-1/2 xl:basis-[35%] transition-opacity duration-300"
                  >
                    <div
                      className="bg-white p-8 rounded-2xl h-full shadow-lg mx-2 transition-all duration-300 hover:shadow-xl select-none"
                      style={{ color: "#0E1A28" }}
                    >
                      <p
                        className="font-bold text-[clamp(26px,_1.75vw,_32px)] leading-[1.155]"
                        style={{
                          fontFamily:
                            fontGuide.headings.h3[i18n.language as "en" | "sr"],
                        }}
                      >
                        {t(`testimonials.testimonial${i}.name`)}
                      </p>
                      <p
                        className="text-[#53758F] text-[clamp(15px,_1.125vw,_20px)] leading-[1.155] mt-2"
                        style={{ fontFamily: fontGuide.body.primary }}
                      >
      ...
      <section
        id="aboutus"
        className="py-[clamp(50px,_2vw,_154px)] scroll-mt-12 bg-white"
      >
        <div className={`${styles.container} mx-auto`}>
          <h2
          ...
          <div className="grid grid-cols-[repeat(18,minmax(0,1fr))] grid-rows-5 max-xs:grid-cols-[repeat(14,minmax(0,1fr))] max-xs:grid-rows-5 gap-x-6 md:grid-rows-3 [&>*]:min-w-0">
            ...
            <div className="row-start-2 row-span-2 col-start-10 col-span-8 max-xs:row-start-3 max-xs:row-span-1 max-xs:col-start-2 max-xs:-col-end-2 md:col-span-6 md:pt-[clamp(74px,_20vw,_296px)] max-xs:pt-2">
              ...
              <p
                className="w-full md:w-[clamp(150px,_25vw,_434px)] text-[#0E1A28] text-[clamp(15px,_1.125vw,_20px)] leading-normal text-justify"
                style={{ fontFamily: fontGuide.body.secondary }}
              >
            ...
            <div className="relative flex flex-row max-xs:flex-col-reverse max-xs:self-center md:flex-col row-start-4 row-span-2 max-xs:row-start-4 md:row-start-1 md:row-span-3 col-start-2 col-end-auto md:col-start-15 md:col-span-5 self-end h-[80%] xl:h-full max-md:h-[500px] max-md:w-[80vw] md:max-w-[407px]">
              ...
              <div className="flex flex-col justify-end relative bottom-0 left-0 bg-white p-6 text-center whitespace-nowrap">
                <p
                  className="text-[clamp(14px,_1.7vw,_24px)] leading-5"
                  style={{ fontFamily: fontGuide.body.secondary }}
                >
                  {parse(t("about.founderName"))}
                </p>
                <p
                  className={`font-['glitten-standard'] text-[#0E1A28] text-[clamp(26px,_3vw,_50px)]`}
                >
                  {parse(t("about.founderPosition"))}
                </p>
              </div>
            </div>
          </div>
      ...
      {/* Trust Section */}
      <section className="pt-12 pb-24 ml:py-24 text-[#0E1A28] bg-white">
        <div className={`${styles.container} mx-auto text-center`}>
          <p
            className="text-[clamp(18px,_1.4vw,_24px)] font-light leading-[1.8] mx-auto mb-[35px]"
            style={{ fontFamily: fontGuide.body.secondary }}
          >
            {parse(t("trust.description"))}
          </p>
          <h2
            ...
            <div className="max-w-full ml:max-w-[clamp(300px,_30vw,_538px)]">
              <div className="mb-[76px]">
                ...
                <p
                  ...
                  style={{ fontFamily: fontGuide.body.secondary }}
                >
                  {parse(t("contact.description"))}
                </p>
              </div>
              <div className="mb-[53px]">
                <h3 className="text-[50px] font-bold leading-none mb-5">
                  {parse(t("contact.address.title"))}
                </h3>
                <a
                  href="https://maps.app.goo.gl/WsJxApFbyGwsxNgj8"
                  className="contact-link"
                  style={{ fontFamily: fontGuide.body.secondary }}
                >
                  {parse(t("contact.address.description"))}
                </a>
              </div>
              <div>
                <h3 className="text-[50px] font-bold leading-none mb-5">
                  {parse(t("contact.phone.title"))}
                </h3>
                <a
                  href="tel:+381603973097"
                  className="contact-link"
                  style={{ fontFamily: fontGuide.body.secondary }}
                >
                  {parse(t("contact.phone.description"))}
                </a>
              </div>
            </div>
            <div
              className="w-full ml:w-1/2 ml:max-w-[772px] mt-[53px] ml:mb-0"
              style={{ fontFamily: fontGuide.body.secondary }}
            >
              <form onSubmit={handleSubmit} className="space-y-[28px]">
                ...
                >
                  {isSubmitting
                    ? parse(t("contact.form.submitting"))
                    : parse(t("contact.form.submit"))}
                </Button>
              </form>
            </div>
          </div>
      ...
      <footer className="bg-[#111111] text-white py-4">
        <div
          className={`${styles.container} flex items-center justify-between max-w-[940px] w-full text-[13px] leading-5 mx-auto text-center ${inter.className}`}
        >
          <p>{parse(t("footer.copyright"))}</p>
          <p>
            {parse(
              (t("footer.info") as string).replace(
                "NTSH",
